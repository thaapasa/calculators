 ...
 */
function core_md5(x: any[], len: number): [number, number, number, number] {
    /* append padding */
    x[len >> 5] |= 0x80 << ((len) % 32)
    x[(((len + 64) >>> 9) << 4) + 14] = len
 ...
 */
function core_hmac_md5(key: string, data: string): [number, number, number, number] {
    let bkey = str2binl(key)
    if (bkey.length > 16) {
        bkey = core_md5(bkey, key.length * chrsz)
    }

    const ipad = [16], opad = [16]
    for (let i = 0; i < 16; i++) {
...
const bl2hHexTab = hexcase ? '0123456789ABCDEF' : '0123456789abcdef'
...
function binl2hex(binarray: number[]): string {
    ...
    for (let i = 0; i < binarray.length * 4; i++) {
        str += bl2hHexTab.charAt((binarray[i >> 2] >> ((i % 4) * 8 + 4)) & 0xF) +
            bl2hHexTab.charAt((binarray[i >> 2] >> ((i % 4) * 8)) & 0xF)
    }
    return str
}

const bl2b64Tab = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
/*
    ...
    for (let i = 0; i < binarray.length * 4; i += 3) {
        ...
        for (let j = 0; j < 4; j++) {
            if (i * 8 + j * 6 > binarray.length * 32) {
                ...
                str += bl2b64Tab.charAt((triplet >> 6 * (3 - j)) & 0x3F)
            }
        }
    }
    return str
